...
const processFile = async (req, res, next) => {
  console.log('File enc controller has been hit!');
  if (!req.file) {
    return res.status(400).json({ message: 'No file was uploaded. Please select a file.' });
  }
  try {
    ...
    // console.log(Received file: ${originalFileName} (${mimeType}));
    // console.log(File size: ${inputFileBuffer.length} bytes);

    if(inputFileBuffer.length > 100000000){
        return res.status(400).json({message:"The entered file was larger than 100mb please try a smaller file"})
    }
    
    const binary_file = encodeFileToBinary(originalFileName,mimeType,inputFileBuffer);
    const processedFileBuffer = encryptBinaryData(binary_file);

  const encryptedFileName = `encrypted-${originalFileName}.txt`;

  // console.log('Sending processed file back for download.');
  // Set Content-Disposition so the browser downloads the file with the proper name
